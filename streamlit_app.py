...
try:
 ...
 raise RuntimeError("ARIMA plugin not available")
try:
 from plugins.prophet_forecast import fit_prophet, forecast_futureexcept Exception:
...
if "bot_running" not in st.session_state:
 st.session_state.bot_running = Falseif "logs" not in st.session_state:
 st.session_state.logs = []
if "rates" not in st.session_state:
 st.session_state.rates = {}
if "history" not in st.session_state:
 st.session_state.history = pd.DataFrame(columns=["Time", "Currency", "Rate", 
"Forecast"])
...
try:
 ...
 except Exception as e:  err = {"time": datetime.now().strftime("%H:%M:%S"), "error": str(e), "trace": 
...
def start_bot():
 ...
 t = threading.Thread(target=bot_loop, args=(st.session_state.bot_queue, stop_event), 
daemon=True)
...
def stop_bot():
 ...
 st.session_state.bot_running = False logger.info("Bot stop requested.")
def drain_bot_queue():
 """Move all items from the bot queue into session_state.logs and history where 
appropriate."""
 drained = 0
...
def fetch_currency_data():
 rates = sample_currency_rates() st.session_state.rates = rates
 return rates
def forecast_rates(rates):
 forecast = {cur: round(val * (1 + random.uniform(-0.05, 0.05)), 2) for cur, val in 
rates.items()}
 return forecast
def update_history(rates, forecast):
 ...
 rows.append({"Time": now, "Currency": cur, "Rate": rates[cur], "Forecast": 
forecast[cur]})
 if rows:
...
pd.DataFrame(rows)], ignore_index=True)
 ...
 st.session_state.history = st.session_state.history.iloc[-
HISTORY_MAX_ROWS:].reset_index(drop=True)
# --- Streamlit UI ---
...
if auto:
interval = st.slider("Refresh interval seconds", 1, 10, 
st.session_state.refresh_interval)
  st.session_state.refresh_interval = interval
drained = drain_bot_queue()   # lightweight sleep to allow UI to update if desired
time.sleep(0.01)
else:
...
width = 0.35
 ax.bar([i - width/2 for i in range(len(x))], current_vals, width=width, alpha=0.7, 
label="Current")
 ax.bar([i + width/2 for i in range(len(x))], forecast_vals, width=width, alpha=0.7, 
label="Forecast")
 ax.set_ylabel("Rate")     ax.set_title("Currency Rates vs Forecast")
...
if not df_cur.empty:
 ...
 ax2.plot(df_cur["Time"], df_cur["Forecast"], linestyle="--", label=f"{cur} 
Forecast")
...
st.table(df_logs)
 except Exception: st.write(st.session_state.logs[-10:])
# Strategy Config Tab
with tabs[1]:
 st.header("Strategy Configuration")
 risk_level = st.slider("Risk Level", 1, 10, 5)
 st.write(f"**Selected Risk Level:** {risk_level}")
 st.write("You can add more strategy parameters here such as stop loss, take profit, 
position sizing rules, and indicators.")
# Logs Tab
with tabs[2]:
...
with tabs[4]:
 ...
 drained = drain_bot_queue() st.write(f"Drained {drained} items from queue.")
# --- Weekly Forecast Tab ---
with tabs[5]:
...
logger.exception("ARIMA error")
  # Prophet forecast
try:
 df_prophet = pd.DataFrame({"ds": pd.to_datetime(df_cur["Time"]), "y": 
df_cur["Rate"].astype(float)})
